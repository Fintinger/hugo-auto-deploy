...
## 12. Git 采用小粒度 + Conventional Commit 规范（AI 协作）

- **日期**：2026-08-27
- **背景**：引入 AI 长期辅助维护后，需要避免混合/大粒度 commit 造成混乱。
- **问题**：AI 与人工改动如何安全、可审查地进入仓库。
- **最终方案**：
  - 小粒度 commit，一次 commit 只表达一个明确目的。
  - 类型划分：`feat`（模板/SCSS/JS/shortcode）、`fix`（修复）、`refactor`（重构）、`docs`（AI 协作文档）、`post`（文章内容）、`chore`（配置）。
  - 不同类型严格分开提交，禁止混合。
  - AI 完成修改后不直接提交，先输出修改摘要 + Git 建议，等待用户确认。
- **原因**：可审查、可回滚、语义清晰；文档/文章/代码隔离避免污染。
- **影响范围**：`docs/DEVELOPMENT_WORKFLOW.md`、`docs/AI_RULES.md`、全部未来 AI 任务。
- **未来注意事项**：历史 emoji 约定（📑✨🛠️）保留作辅助标记；提交前必须 `git status` + `git diff --cached` 检查暂存范围。
